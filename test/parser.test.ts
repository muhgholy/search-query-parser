     ...
     describe('parse', () => {
          ...
          describe('operators', () => {
               ...
               it('should parse header-k operator', () => {
                    ...
                    expect(result).toHaveLength(1)
                    expect(result[0]).toEqual({ type: 'header-k', value: 'X-Custom', negated: false })
               })

               it('should parse header-v operator', () => {
                    const result = parse('header-v:custom-value')
                    expect(result).toHaveLength(1)
                    expect(result[0]).toEqual({ type: 'header-v', value: 'custom-value', negated: false })
               })
          ...
          describe('custom operators', () => {
               ...
               it('should override default operators', () => {
                    const result = parse('from:me', {
                         operators: [
                              { name: 'from', aliases: [], type: 'string', allowNegation: true }
                         ]
                    })
